...
if ( ! function_exists( 'specialty_setup' ) ) :
	function specialty_setup() {
		...
		specialty_migrate_typography_mods();
	}
endif;
...
function specialty_enqueue_scripts() {
	...
	wp_register_style( 'specialty-google-font', specialty_fonts_url(), array(), null );
	wp_register_style( 'specialty-typography-google', get_theme_file_uri( '/css/typography-gfont.css' ), array(), specialty_asset_version() );
	wp_register_style( 'specialty-typography-default', get_theme_file_uri( '/css/typography-default.css' ), array(), specialty_asset_version() );
	...
	if ( get_theme_mod( 'global_typo_is_google_active', specialty_customizer_defaults( 'global_typo_is_google_active' ) ) ) {
		...
		$main_dependencies = array_merge( $main_dependencies, array(
			...
			'specialty-typography-google',
		) );
	} else {
		$main_dependencies = array_merge( $main_dependencies, array(
			'specialty-typography-default',
		) );
	}
	...
	if ( is_child_theme() ) {
		wp_enqueue_style( 'specialty-style-parent', get_template_directory_uri() . '/style.css', $main_dependencies, specialty_asset_version() );
	}

	wp_enqueue_style( 'specialty-style', get_stylesheet_uri(), $main_dependencies, specialty_asset_version() );

	wp_add_inline_style( 'specialty-style', specialty_get_all_customizer_css() );
...
function specialty_migrate_typography_mods() {
	...
	foreach ( $sizes as $old => $new ) {
		if ( get_theme_mod( $old ) ) {
			$new_mod = specialty_typography_control_defaults_empty_breakpoints( array(
				'desktop' => array(
	...
	if ( 1 === intval( get_theme_mod( 'lowercase_widget_titles' ) ) ) {
		$new_mod   = get_theme_mod( 'global_typo_widget_titles' );
		$transform = 'none';
		if ( is_array( $new_mod ) && isset( $new_mod['desktop']['transform'] ) ) {
			$new_mod['desktop']['transform'] = $transform;
		} else {
			$new_mod = specialty_typography_control_defaults_empty_breakpoints(
				array(
					'desktop' => array(
						'transform' => $transform,
					),
		...
		set_theme_mod( 'global_typo_widget_titles', $new_mod );
...
function specialty_customizer_defaults( $setting = false ) {
	$theme = wp_get_theme();

	// phpcs:disable WordPress.Arrays.MultipleStatementAlignment.DoubleArrowNotAligned
	$defaults = apply_filters(
		'specialty_customizer_defaults',
		array(
			'global_typo_is_google_active' => 1,
	...
	if ( ! empty( $setting ) && array_key_exists( $setting, $defaults ) ) {
		return apply_filters( 'specialty_customizer_default', $defaults[ $setting ], $setting );
	}
