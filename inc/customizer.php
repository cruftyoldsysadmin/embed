...
function specialty_customize_register( $wpc ) {
	...
	$wpc->selective_refresh->add_partial( 'theme_style', array(
		'selector'            => '#specialty-style-inline-css',
		'render_callback'     => 'specialty_get_all_customizer_css',
		'settings'            => array_keys( specialty_get_registered_typography_controls() ),
		'container_inclusive' => false,
	) );

	$wpc->selective_refresh->add_partial( 'theme_gfonts', array(
		'selector'            => '#specialty-user-google-fonts-css',
		'render_callback'     => 'specialty_customize_preview_google_fonts',
		'settings'            => array_keys( specialty_get_registered_typography_controls() ),
...
function specialty_customize_register_custom_controls( $wpc ) {
	...
	$wpc->register_control_type( 'Specialty_Customize_Typography_Control' );
}

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function specialty_customize_preview_js() {

	// Generic preview code.
	wp_enqueue_script( 'specialty-customizer-preview', get_theme_file_uri( '/inc/customizer/preview/preview.js' ), array( 'jquery' ), specialty_asset_version(), true );

	// Options-specific preview code.
	wp_enqueue_script( 'specialty-customizer-preview-global', get_theme_file_uri( '/inc/customizer/preview/global.js' ), array( 'jquery' ), specialty_asset_version(), true );
}
add_action( 'customize_preview_init', 'specialty_customize_preview_js' );

function specialty_customize_controls_js() {
	wp_enqueue_style( 'specialty-customizer-controls', get_theme_file_uri( '/inc/customizer-controls/style.css' ), array(), wp_get_theme()->get( 'Version' ) );
}
add_action( 'customize_controls_enqueue_scripts', 'specialty_customize_controls_js' );

function specialty_typography_control_defaults_empty_breakpoints( $override_breakpoints = array() ) {
	$defaults = array(
		'desktop' => array(
			'family'     => '',
			'variant'    => '',
...
function specialty_sanitize_typography_control_breakpoints( $values ) {
	$defaults = specialty_typography_control_defaults_empty_breakpoints();
	...
	foreach ( $values as $breakpoint => $breakpoint_values ) {
		if ( array_key_exists( $breakpoint, $defaults ) ) {
			...
			$new_breakpoint_values['size']       = specialty_sanitize_intval_or_empty( $new_breakpoint_values['size'] );
			$new_breakpoint_values['lineHeight'] = specialty_sanitize_intval_or_empty( $new_breakpoint_values['lineHeight'] );
			$new_breakpoint_values['transform']  = sanitize_text_field( $new_breakpoint_values['transform'] );
			$new_breakpoint_values['spacing']    = specialty_sanitize_floatval_or_empty( $new_breakpoint_values['spacing'] );
...
if ( ! function_exists( 'specialty_customize_preview_google_fonts' ) ) {
	function specialty_customize_preview_google_fonts( $_this, $container_context ) {
		specialty_enqueue_google_fonts();
		wp_print_styles( 'specialty-user-google-fonts' );
	}
}
 ...
 * CSS Generator.
 */
require_once get_theme_file_path( '/inc/customizer/class-specialty-customizer-css-generator.php' );
